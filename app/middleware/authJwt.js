...
isOrganisateurdevote = (req, res, next) => {
  User.findByPk(req.userId).then((user) => {
    user.getRoles().then((roles) => {
      for (let i = 0; i < roles.length; i++) {
        if (roles[i].name === 'organisateurdevote') {
          next()
          return
        }
      }
      res.status(403).send({
        message: 'Require organisateurdevote Role!',
      })
    })
  })
}
...
const authJwt = {
  ...
  isOrganisateurdevote: isOrganisateurdevote,
